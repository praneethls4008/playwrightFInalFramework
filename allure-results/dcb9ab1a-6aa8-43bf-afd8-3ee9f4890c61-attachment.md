...
```ts
  ...
  32 |         const [loginPage]  = await Promise.all([
> 33 |           context.waitForEvent('page'),
     |                   ^ Error: browserContext.waitForEvent: Test ended.
  34 |           headerComponent.clickLoginOrSignupBtn()
  35 |         ]);
  36 |         await expect.soft(loginPage, 'Login btn redirection is not as expected').toHaveURL(QAPlaygroundHeaderData.loginBtnHref);
  37 |       }
  38 |     )
  39 | }
  40 | 
