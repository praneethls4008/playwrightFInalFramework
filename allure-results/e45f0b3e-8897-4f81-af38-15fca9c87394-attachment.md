...
```ts
  ...
  30 |       async() => {
  31 |         const context = page.context();
> 32 |         const loginPagePromise = context.waitForEvent('page');
     |                                          ^ Error: browserContext.waitForEvent: Test ended.
  33 |         await headerComponent.clickLoginOrSignupBtn();
  34 |         const loginPage= await loginPagePromise;
  35 |         await expect.soft(loginPage, 'Login btn redirection is not as expected').toHaveURL(QAPlaygroundHeaderData.loginBtnHref);
  36 |       }
  37 |     )
  38 | }
  39 | 
